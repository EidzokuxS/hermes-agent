...
param(
    ...
    # builds apps/desktop into a launchable Nox.exe.
    #
    # Why opt-in:
    ...
    #     from inside an already-launched Nox.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Nox.exe
...
$RepoUrlSsh = "git@github.com:EidzokuxS/hermes-agent.git"
$RepoUrlHttps = "https://github.com/EidzokuxS/hermes-agent.git"
$PythonVersion = "3.11"
...
function Install-Desktop {
    # Build apps/desktop into a launchable Nox.exe. Only called from
    ...
    # resolves apps/desktop/release/win-unpacked/Nox.exe directly,
    ...
    # The --dir mode produces an unpacked Nox.exe in
    ...
    # are NOT trying to work around). The Nox icon + product name are
    # stamped onto Nox.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Nox.exe",
        "$desktopDir\release\win-arm64-unpacked\Nox.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Nox.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Nox icon + identity are stamped onto Nox.exe by the
    ...
    #    Nox.exe. We deliberately do NOT point them at `hermes desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Nox.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Nox.lnk')
        )
        ...
        # with the Nox icon, but without this the shortcut can keep drawing
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Preparing Nox source";                 Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
