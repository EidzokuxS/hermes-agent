...
# Configuration
REPO_URL_SSH="git@github.com:EidzokuxS/hermes-agent.git"
REPO_URL_HTTPS="https://github.com/EidzokuxS/hermes-agent.git"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Nox.app)"
...
install_desktop() {
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Nox" ]; then
            app="$desktop_dir/release/linux-unpacked/Nox"
        elif [ -x "$desktop_dir/release/linux-unpacked/nox" ]; then
            app="$desktop_dir/release/linux-unpacked/nox"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Nox.app" \
            "$desktop_dir/release/mac/Nox.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
