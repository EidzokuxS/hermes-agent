 ...
 *
 * Stamps the Nox icon + identity onto the packed Windows Nox.exe via
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Nox')
 */
...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Nox'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Nox.exe keeps the stock Electron icon`)
  }
}
