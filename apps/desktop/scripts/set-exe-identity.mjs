#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Nox icon + version metadata onto the
// built Nox.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Nox.exe keeps the stock Electron icon and
...
//   node scripts/set-exe-identity.mjs <path-to-Nox.exe>
//
...
// Stamp the Nox icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'Nox',
      FileDescription: 'Nox',
      CompanyName: 'Nox contributors',
      InternalName: 'Nox',
      LegalCopyright: 'Copyright (c) 2026 Nox contributors',
      OriginalFilename: 'Nox.exe'
    }
  })

  console.log('[set-exe-identity] done — Nox icon + identity stamped')
}
