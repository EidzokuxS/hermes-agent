 ...
 *
 *   macOS:   …/Nox.app/Contents/MacOS/Nox  → …/Nox.app
 *   Windows: …\Nox\Nox.exe                 → …\Nox  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Nox.app/Contents/MacOS/Nox → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Nox.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Nox.exe directly in the install dir.
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Nox install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`
