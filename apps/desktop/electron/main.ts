...
const USER_DATA_OVERRIDE = process.env.NOX_DESKTOP_USER_DATA_DIR || process.env.HERMES_DESKTOP_USER_DATA_DIR
...
// The embedded backend still consumes HERMES_HOME, but the product owns the
// directory it points at. New installs use %LOCALAPPDATA%\nox or ~/.nox and do
// not silently adopt an existing Nox profile. NOX_HOME is the explicit
// override; test user-data overrides remain sandboxed below their temp root.
const HERMES_HOME = resolveProductHome({
  env: process.env,
...
// ACTIVE_HERMES_ROOT — the canonical mutable Nox install. Same path
...
// active-profile.json records which Nox profile the desktop launches its
...
const PRODUCT_CAPTURE_DIR = String(process.env.NOX_DESKTOP_CAPTURE_DIR || '').trim() || null
const PRODUCT_CAPTURE_DELAY_MS = (() => {
  const raw = Number.parseInt(String(process.env.NOX_DESKTOP_CAPTURE_DELAY_MS || ''), 10)
...
const APP_NAME = process.env.NOX_DESKTOP_APP_NAME || process.env.HERMES_DESKTOP_APP_NAME || PRODUCT_NAME
...
// Seed the native About panel with the live Nox version. This is refreshed
...
app.setAboutPanelOptions({
  ...
  copyright: 'Nox contributors · Nox Agent © Nous Research'
})
...
let bootProgressState = {
  ...
  message: 'Waiting to start Nox backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
async function waitForUpdateToFinish() {
  ...
  while (marker && Date.now() < deadline) {
    await advanceBootProgress(
      'backend.update-wait',
      'An update is finishing — Nox will start automatically when it completes…',
      12
    )
...
function unwrapWindowsVenvHermesCommand(command, backendArgs) {
  ...
  if (
    ...
    rememberLog(
      `Ignoring venv Nox at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Nox Python at ${python}`,
    command: python,
...
function findSystemPython() {
  ...
  // CPython but several Nox deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Nox' terminal tool requires
...
// `hermes update`). Instead we spawn the staged Nox-Setup binary with
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Nox — this window will close and the updater will open. Don’t reopen Nox yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    if (!lock.unlocked) {
      ...
      const message =
        'Update aborted: another process is holding the Nox install open ' +
        '(a second Nox window or a terminal running hermes?). Close it and retry.'
...
async function applyUpdatesPosixInApp(opts: any) {
  ...
  // Put the Nox-managed Node and the venv on PATH so `hermes desktop`'s
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Nox (git + dependencies)…', percent: 10 })
  ...
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Nox to retry.',
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Nox…', percent: 100 })
      ...
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Nox to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Nox desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Nox to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Nox.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Nox.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Nox to load the new version.',
      percent: 100
    })
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Nox to load the new version.',
      percent: 100
    })
...
function resolveHermesCwd() {
  ...
  // `…/win-unpacked` on Windows or `/Applications/Nox.app/Contents/...`
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Nox at ${ACTIVE_HERMES_ROOT}`,
    command,
    args: ['-m', 'hermes_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      hermesHome: HERMES_HOME,
...
function resolveHermesBackend(backendArgs) {
  ...
  if (overrideRoot && isHermesSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Nox source at ${overrideRoot}`, backendArgs)
  ...
  if (!IS_PACKAGED && isHermesSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Nox source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  if (useUnmanagedRuntime) {
    ...
    if (hermesOverride) {
      ...
      if (resolvedOverride) {
        ...
        rememberLog(`Ignoring Windows Nox override under WSL: ${hermesOverride}`)
      }
    } else {
    ...
    if (hermesCommand) {
      if (looksLikeDesktopAppBinary(hermesCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Nox CLI: ${hermesCommand}`)
        hermesCommand = null
      }
    }
    ...
    if (hermesCommand) {
      ...
      if (verifyHermesCli(hermesCommand, { shell: shellForProbe })) {
        return (
          unwrapWindowsVenvHermesCommand(hermesCommand, backendArgs) || {
            label: `existing Nox CLI at ${hermesCommand}`,
      ...
      rememberLog(
        `Ignoring existing Nox CLI at ${hermesCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }
  ...
  return {
    ...
    label: 'Nox Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Nox install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Nox recovery was handed off to Nox Setup. The desktop will restart when recovery completes.'
      )
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Nox install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Nox bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
  ...
  if (!isHermesSourceRoot(ACTIVE_HERMES_ROOT)) {
    throw new Error(
      `Nox install at ${ACTIVE_HERMES_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Nox' terminal tool calls bash.exe
  ...
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Nox on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    throw new Error(
      `Nox venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Nox at ${ACTIVE_HERMES_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Nox runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Nox backend URL protocol: ${parsed.protocol}`))
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Nox backend.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Nox backend after ${timeoutMs}ms`))
    })
...
async function waitForHermes(baseUrl, token) {
  ...
  while (Date.now() < deadline) {
    try {
      await probeBackendReadiness(fetchJson, baseUrl, token)
  ...
  throw new Error(`Nox backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
// signal so the chat composer doesn't stay stuck on "Starting Nox...".
...
//
// Hosted Nox gateways gate the dashboard behind an OAuth provider (e.g.
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Hermes Cloud agent…' : 'Sign in to Nox gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Nox backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// so a single override flips every Nox surface to the same portal.
...
// authenticates via PRIVY, not the Nox gateway session cookies, so this
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Nox gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote Nox gateway is selected, but no session token is saved. ' +
    ...
    if (!rawEnvToken) {
      throw new Error(
        'HERMES_DESKTOP_REMOTE_URL is set but HERMES_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Nox backend.'
      )
    }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Nox
...
async function testDesktopConnectionConfig(input: any = {}) {
  ...
  // connect to Nox gateway". Mirror the renderer's connect here so the test
...
async function spawnPoolBackend(profile, entry) {
  ...
  rememberLog(`Starting Nox backend for profile "${profile}" via ${backend.label}`)
  ...
  child.once('error', error => {
    rememberLog(`Nox backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Nox backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)

    if (!ready) {
      rejectStart?.(
        new Error(`Nox backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    ...
    label: `Nox backend for profile "${profile}"`,
    rememberLog
  })
...
async function startHermes() {
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Nox backend', 8)
    // Resolve for the desktop's primary profile so a per-profile remote
    // override on the active profile is honored (falls back to env / global).
    const remote = await resolveRemoteBackend(primaryProfileKey())

    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Nox backend at ${remote.baseUrl}`, 24)
      await waitForHermes(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Nox backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Nox runtime', 28)
    const backend = await ensureRuntime(resolveHermesBackend(backendArgs))
    // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
    backend.args = getBackendArgsForRuntime(backend)
    const hermesCwd = resolveHermesCwd()
    ...
    await advanceBootProgress('backend.spawn', `Starting Nox backend via ${backend.label}`, 84)
    rememberLog(`Starting Nox backend via ${backend.label}`)
    ...
    hermesProcess.once('error', error => {
      rememberLog(`Nox backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Nox backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    hermesProcess.once('exit', (code, signal) => {
      rememberLog(`Nox backend exited (${signal || code})`)
      hermesProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Nox backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Nox backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentHermesLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Nox backend to launch', 86)
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Nox backend to become ready', 90)
    ...
    updateBootProgress({
      phase: 'backend.ready',
      message: 'Nox backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Nox is
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Nox app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Nox icon drops out of cmd/alt-tab — especially when the
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Nox in the normal
...
const PRODUCT_CAPTURE_SIZES = [
  { height: 900, label: '1440x900', width: 1440 },
  { height: 700, label: '900x700', width: 900 },
  { height: WINDOW_MIN_HEIGHT, label: `${WINDOW_MIN_WIDTH}x${WINDOW_MIN_HEIGHT}`, width: WINDOW_MIN_WIDTH }
]

async function captureProductScreenshots(win, outputDirectory) {
  try {
    ...
    await loaded
    await sleep(PRODUCT_CAPTURE_DELAY_MS)
    await fs.promises.mkdir(outputDirectory, { recursive: true })

    for (const size of PRODUCT_CAPTURE_SIZES) {
      win.setSize(size.width, size.height, false)
      await sleep(250)

      const image = await win.webContents.capturePage()
      await fs.promises.writeFile(path.join(outputDirectory, `${size.label}.png`), image.toPNG())
    }

    rememberLog(`Product screenshots captured in ${outputDirectory}`)
  } catch (error) {
    process.exitCode = 1
    rememberLog(`Product screenshot capture failed: ${error instanceof Error ? error.stack || error.message : error}`)
  } finally {
    app.quit()
  }
}
...
function createWindow() {
  ...
  mainWindow.once('ready-to-show', () => {
    if (mainWindow && !mainWindow.isDestroyed()) {
      if (PRODUCT_CAPTURE_DIR) {
        void captureProductScreenshots(mainWindow, PRODUCT_CAPTURE_DIR)
      } else {
        mainWindow.show()
      }
    }
...
// "Starting Nox…". Before the renderer's backoff loop reconnects, it asks us
...
ipcMain.handle('hermes:connection:revalidate', async () => {
  ...
  try {
    ...
    rememberLog('Cached remote Nox backend failed liveness probe; dropping stale connection.')
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Nox' TUI into
...
// Resolve the canonical Nox version (the one `release.py` bumps in
// hermes_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Nox version instead of the Electron app's own package.json version,
...
// Re-resolve the live Nox version and push it into the native About panel
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Nox agent venv at ${VENV_ROOT}.`
    }
  }
...
// ---------------------------------------------------------------------------
const HERMES_PROTOCOL = PRODUCT_PROTOCOL
