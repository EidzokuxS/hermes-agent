...
export const boundedIdSchema = z.string().min(1).max(128)
...
const externalInterfaceProvenanceSchema = z
  .object({
    clientEventId: boundedIdSchema,
    interfaceOwnerId: boundedIdSchema,
    kind: z.literal('external-interface')
  })
  .strict()

const continuationProvenanceSchema = z
  .object({
    continuationId: boundedIdSchema,
    kind: z.literal('continuation'),
    scheduledByActId: boundedIdSchema
  })
  .strict()

const bootstrapProvenanceSchema = z
  .object({
    bootstrapId: boundedIdSchema,
...
const cortexProvenanceSchema = z
  .object({
...
export const provenanceSchema = z.discriminatedUnion('kind', [
  ...
  continuationProvenanceSchema,
  cortexProvenanceSchema,
  externalInterfaceProvenanceSchema,
  runtimeProvenanceSchema
])

export const causalLinksSchema = z
  .object({
    actId: boundedIdSchema.optional(),
    causeSequences: z.array(z.number().int().positive()).max(32),
    eventId: boundedIdSchema.optional()
  })
  .strict()

export type CausalLinks = z.infer<typeof causalLinksSchema>
export type Provenance = z.infer<typeof provenanceSchema>

function orderJson(value: JsonValue): JsonValue {
  if (Array.isArray(value)) {
    return value.map(item => orderJson(item))
  }
  ...
  if (value !== null && typeof value === 'object') {
    return Object.fromEntries(
      Object.entries(value)
        ...
        .map(([key, item]) => [key, orderJson(item)])
    )
  }
...
export function canonicalize(value: unknown): JsonValue {
  return orderJson(jsonValueSchema.parse(value))
}
...
function sha256(value: string): string {
  ...
  new DataView(bytes.buffer).setBigUint64(paddedLength - 8, bitLength, false)
  ...
  for (let offset = 0; offset < bytes.length; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temporary1 = ((h ?? 0) + upperSigma1 + choice + (SHA256_ROUND[index] ?? 0) + (words[index] ?? 0)) >>> 0
