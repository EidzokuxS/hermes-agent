...
the Python agent or the user's config/data:
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Nox.app`` or ``~/Applications/Nox.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Nox`` (NSIS per-user)
...
the app name ("Nox"), separate from ``$HERMES_HOME``:
  - macOS:   ``~/Library/Application Support/Nox``
  - Windows: ``%APPDATA%\\Nox``
  - Linux:   ``$XDG_CONFIG_HOME/Nox`` (default ``~/.config/Nox``)
...
def desktop_userdata_dir() -> Path:
    ...
    Mirrors Electron's ``app.getPath('userData')`` for an app named "Nox"
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the Nox product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Nox.app"),
            home / "Applications" / "Nox.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Nox).
            local_base / "Programs" / "Nox",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Nox")
    else:
        ...
        paths += [
            data_base / "applications" / "nox.desktop",
            data_base / "applications" / "Nox.desktop",
        ]
    return paths
