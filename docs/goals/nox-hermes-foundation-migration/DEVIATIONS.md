...
## D-013 — Final security evidence supersedes the dependency freeze

- **Recorded:** 2026-07-13
- **Plan location:** Task 7 dependency/security scan and the Task 0 root-manifest freeze.
- **Evidence:** The locked production environment passed compatibility checks, but a fresh OSV-backed `pip-audit` found 13 published vulnerabilities in four reachable packages: multipart parsing (`python-multipart==0.0.27`), ASGI request/form/static handling (`starlette==1.0.1`), untrusted image processing (`Pillow==12.2.0`) and credential/crypto paths (`cryptography==46.0.7`). The corresponding product paths import and use those packages; this is not an unused-development-only finding.
- **Decision:** Permit one bounded Python dependency refresh before final evidence: `python-multipart==0.0.32`, `starlette==1.3.1`, `Pillow==12.3.0` and `cryptography==48.0.1`. Keep FastAPI and all Node manifests unchanged, regenerate only `uv.lock`, synchronize matching lazy-install declarations and security regression floors, then rerun compatibility, vulnerability, focused backend, Nox, Desktop and packaged-journey gates. Resolving `cryptography==48.0.1` necessarily selects `alibabacloud-tea-openapi==0.3.16` for the optional DingTalk extra because its newer `0.4.4` release caps cryptography below 47; this solver-visible transitive substitution is included in the same regression run.
- **Why this preserves intent:** The freeze exists to prevent migration drift, not to ship known reachable vulnerabilities discovered by the plan's own final gate. The change is limited to patched releases, keeps the architecture and product behavior fixed, and receives stricter validation than the original baseline.
- **Rollback:** Do not roll back to a vulnerable lock. If a patched release proves incompatible, select another advisory-fixed compatible version and record it here before rebuilding evidence.

## D-014 — Packaged Nox owns and proves its live backend

- **Recorded:** 2026-07-13
- **Plan location:** Tasks 5 and 7, side-by-side product data and target-perspective packaged acceptance.
- **Evidence:** A new conversation in the released `Nox.exe` identified itself as Hermes Agent. Process and log inspection proved that the packaged Desktop had selected `C:\Users\robra\AppData\Local\hermes\hermes-agent\venv\Scripts\python.exe` from `PATH` because the Nox-owned runtime was absent. That unmanaged checkout contains neither `nox.identity` nor `identity/NOX.md`. The prior packaged journey used a source override and therefore proved the repository implementation while missing the ordinary resolver path.
- **Decision:** Packaged Nox may launch only its side-by-side product runtime or an explicit developer source override. Unmanaged `hermes` commands and system Python remain development conveniences but are not packaged fallbacks. Runtime readiness validates `nox.identity.load_nox_identity()` in addition to Hermes dependencies. Task 7 release evidence is superseded until a clean packaged launch records the selected backend provenance and exercises a new Nox session without repository `cwd` or source override.
- **Why this preserves intent:** The change enforces the already accepted side-by-side policy and the single Hermes model loop while preventing a differently versioned Hermes installation from replacing Nox's accepted identity and causal hooks.
- **Rollback:** None while Nox identity and causal integration are product invariants. A future external-backend mode requires an explicit compatibility handshake and separate user-visible selection.
